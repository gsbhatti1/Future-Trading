...
useMaFilter = input.bool(title='Use Moving Average for Filtering (Current Timeframe)?', defval=false, group = "moving average filtering",
  tooltip = "'If the trend is your friend, follow it' - This will allow you to use a moving average on the current timeframe as an additional filter.")
maCurType = input.string(defval='SMA', options=['EMA', 'SMA'], title='MA Type For Filtering (Current Timeframe)', group = "moving average filtering")
maCurLength = input.int(defval=50, title="Moving Average:    Length", minval=1, group = "moving average filtering")
maCurColorR = input(color.new(color.green, 50), title = " Rising", group = "moving average filtering")
maCurColorF = input(color.new(color.red, 50), title = " Falling", group = "moving average filtering")

useMaFilterHt = input.bool(title='Use Moving Average for Filtering (High Timeframe)?', defval=false, group = "moving average filtering",
  tooltip = "'If the trend is your friend, follow it' - This will allow you to use a moving average on the high timeframe as an additional filter.")
maHType = input.string(defval='SMA', options=['EMA', 'SMA'], title='MA Type For Filtering (High Timeframe)', group = "moving average filtering")
maHLength = input.int(defval=50, title="Moving Average:    Length", minval=1, group = "moving average filtering")
maHColorR = input(color.new(color.green, 50), title = " Rising", group = "moving average filtering")
maHColorF = input(color.new(color.red, 50), title = " Falling", group = "moving average filtering")

// ADR Filtering Inputs
useAdrFilter = input.bool(title='Use ADR for Filtering?', defval=false, group = "adr filtering",
  tooltip = "'If the trend is your friend, follow it' - This will allow you to use an ADR filter on the current timeframe as an additional filter.")
adrPeriod = input.int(defval=120, title="% of ADR Value", minval=1, group = "adr filtering")
adrVisible = input.string(defval='Bottom', options=['Bottom', 'Top'], title="ADR Table Visibility", group = "adr filtering")

// === STRATEGY LOGIC ===

// Implement the donchian breakout strategy
dcHigh = ta.highest(high, dcPeriodHigh)
dcLow = ta.lowest(low, dcPeriodLow)

if (triggerInput == 'Wick')
    if not na(dcHigh[1]) and not na(dcLow[1])
        if close > dcHigh[1]
...
maFilterSlope = ta.sma(close, maSlopeLength)
if useMaFilterSlope and (maFilterSlope > maCurType == 'SMA' ? ta.sma(close, maCurLength) : ta.ema(close, maCurLength)) 
    strategy.entry('Buy', strategy.long)

// Implement the high timeframe moving average filter
if useMaFilterHt and (ta.sma(close, maHLength) > maHType == 'SMA' ? ta.sma(close, maHLength) : ta.ema(close, maHLength))
    strategy.exit('Sell', from_entry='Buy')

// Implement the ADR filter
adr = (close - low[21]) / 21
if useAdrFilter and adr > adrPeriod / 100 * (dcHigh - dcLow)
    strategy.entry('Buy', strategy.long)

// Plotting
plot(dcHigh, color=upperColor, title='Upper Donchian Channel')
plot(dcLow, color=lowerColor, title='Lower Donchian Channel')
fill(dcHigh, dcLow, color=fillColor, title='Donchian Channel Fill')

// ADR Table
table.add_row('Current Close', close)
table.add_row('21 Day Low', low[21])
table.add_row('ADR', adr)
table.add_row('% of ADR Value', 100 * (adr / (dcHigh - dcLow)))
table.set_plot(table, 'Bottom')
```

This is the translated Pine Script with all code blocks and formatting preserved. Please note that some minor adjustments were made to ensure clarity in English.