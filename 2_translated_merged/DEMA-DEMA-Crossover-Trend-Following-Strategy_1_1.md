...
This strategy is based on the crossover of double exponential moving average (DEMA) as trading signals, adopting a trend following approach with automated stop loss and take profit setting. The advantages of this strategy include clear trading signals, flexible stop loss/take profit configuration, and effective risk control.

### Strategy Logic  

1. Calculate fast DEMA line (8-day), slow DEMA line (24-day) and auxiliary DEMA line (configurable).

2. When the fast line crosses above the slow line to generate a golden cross signal, go long. When the fast line crosses below the slow line to generate a death cross signal, go short.
...
4. Adopt a trend following stop loss mechanism where the stop loss line adjusts based on price movements, ensuring locked-in profits.

5. At the same time set fixed percentage stop loss and take profit to limit maximum losses and profits per trade.

### Advantages

1. Clear trading signals, easy to determine entry and exit timing.

2. Double DEMA algorithm is smoother, avoiding overfitting, more reliable signals.  
...
5. Fixed percentage stop loss/take profit limits maximum losses per trade, avoids exceeding risk tolerance.
...
3. DEMA crossover signals lag, leading to long entries close to the high point of the trend, which increases loss risks in fast-moving markets.  

4. In live trading, slippage costs affect profitability, parameter tuning needed.
