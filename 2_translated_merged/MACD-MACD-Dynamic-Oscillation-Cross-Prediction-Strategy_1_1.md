...
//@version=5
strategy(title="MACD Dynamic Oscillation Cross Prediction Strategy", shorttitle="MACD", commission_type=strategy.commission.percent, commission_value=0.1, slippage=3, default_qty_type=strategy.percent_of_equity, default_qty_value=100)
...
// Date inputs
start_date = input.time(title="Start Date", defval=timestamp("2018-01-01T00:00:00"))
end_date = input.time(title="End Date", defval=timestamp("2069-12-31T23:59:59"))
...
// Strategy logic
isInDateRange = not na(start_date) and time >= start_date and (na(end_date) or time <= end_date)
...
anticipate_long = isInDateRange and hist < 0 and hist_change > 0 and hist[2] <= 0

if (anticipate_long)
    strategy.entry("Long", strategy.long)

// Anticipate a bearish crossover: histogram is positive, decreasing, and approaching zero
anticipate_short = isInDateRange and hist > 0 and hist_change < 0 and hist[2] >= 0
