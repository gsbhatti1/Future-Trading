...
/*
-- The strategy references this template and directly calls the $.Test() method to invoke this function.
-- main is not triggered in strategies, it's only used as an entry point for debugging templates.
*/
...
$.GetAmountPrecision = function (jiaoyisuo, bidui) {
    ...
    if (isBinance || (unknown && !item)) {
        ...
        if (item) {
            //Binance data is incorrect, eth 6 digits, btc 7 digits, while the list shows 8 digits
            var binanceP = getPrecision(item.minamount);
