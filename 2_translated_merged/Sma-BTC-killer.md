...
The strategy uses 3 different SMAs (7, 21, 55) to find the correct trend.
...
CLOUD - This is one of the newest indicators I'm using. This indicator helps the strategy by indicating the correct market trend. By applying the great length of this indicator, I am able to notice a change in the trend a little later, but more accurately.

Additionally, I added trailing stop-loss for maximum security.

To be honest, this strategy looks really good. It has many trades, high profit, and only a small number of indicators. The future profits could be similar.

Using this combination of SMAs gives me amazing quick changes while the trend is also changing fast like here:
...
calcADX(_len) =>
    up              = change(high)
	down            = change(low)
    // Add the rest of your code for calculating ADX here
```

Note: The `calcADX` function is incomplete in the original script, and it should be completed with the necessary logic to calculate the ADX.