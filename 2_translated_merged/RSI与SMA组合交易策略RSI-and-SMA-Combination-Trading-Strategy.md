...
```pinescript
//@version=5
strategy("RSI with SMA Combination Trading Strategy", overlay=true)

lengthRSI = input.int(14, "RSI length")
fastSMA = ta.sma(close, 100)
slowSMA = ta.sma(close, 150)

rsiValue = ta.rsi(close, lengthRSI)

longCondition = rsiValue > 50 and fastSMA > slowSMA
shortCondition = rsiValue < 50 and fastSMA < slowSMA
...
if (strategy.position_size == strategy.long and not longCondition)
    strategy.close("Long")

if (strategy.position_size == strategy.short and not shortCondition)
