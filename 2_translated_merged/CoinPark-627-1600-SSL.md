...
CoinPark Exchange Universal Protocol - 627-1600 - Update - SSL Validation Closed
...
## CoinPark Universal Protocol for Python 2.7
Purpose: To enable support for the coinpark.cc exchange with BotVs.
Code Repository:
...
### 1. Run Plugin on Host Server

Upload and save the file to your host server, then run it in the background:

    nohup python coinpark.py & 
You can also specify a port when running:

    python coinpark.py 8866
It can be run as a regular robot on the simulation disk without any cost.

### 2. Configure Exchange on FMZ Website

Exchange configuration:
![](https://dn-filebox.qbox.me/c1456599386e597f2966f67b49c7102bab857fef.png)
Since the account has no assets, detailed testing was not conducted. Feedback on bugs is welcome.
Updated June 26, 2018, 15:57 - Bug fixes.
...
# -*- coding: utf-8 -*-
'''
CoinPark Universal Protocol using Python 2.7
Run Address: http://127.0.0.1:6667, port can be specified.
Since the account has no assets, detailed testing was not conducted. Feedback on bugs is welcome.
QQ:1051804485
Feedback Address: https://www.botvs.com/bbs-topic/1963
Updated June 26, 2018, 15:57 - Bug fixes.
The universal protocol can be run as a regular robot on the BotVs simulation disk without any cost.
To use IO functions, you need to overload the `rpc` method in the exchange. Here is an example in JS:
```javascript
