...
* Based on Inventor's powerful low-level framework, fully supports digital currencies spot and futures as well as domestic commodity futures
* Compatibility work will continue; currently at 90% compatibility, most strategies can run without modification
* API documentation: https://www.fmz.com/bbs-topic/2569

> Language Enhancement

InventorQuantum not only implements the Ma language interpreter but also enhances it to allow mixing with advanced languages like JavaScript. Here's an example:

```
%%
// This can call any InventorQuantum API
scope.TEST = function(obj) {
    return obj.val * 100;
}
%%
Close: C;
Close magnified by 100 times: TEST(C);
Previous Close magnified by 100 times: TEST(REF(C, 1)); // Hovering over the backtest K-line will display variable values
```
...
In 1994, Covel picked up an issue of Financial World and skimmed through an article titled "Wall Street's Top Players." Among famous investors like George Soros and Julian Robertson, Covel noticed a name he did not recognize at 25th on the list: R. Jerry Parker, who stated that he was trained as a "Turtle" by Richard Dennis (another name Covel did not recognize). Parker was the only investor in the top hundred advertised as being "trained," and as an investor himself, Covel found this story intriguing.
...
In capturing trend signals, the Turtle Trading Law uses a very important technical indicator, the Donchian channel. This channel is very similar to the familiar Bollinger Bands but is somewhat different in terms of specific calculations.

Richard Donchian invented this indicator. It consists of three curves of different colors. The indicator uses the highest price within the period (usually 20; some platform system settings can be changed, some cannot) and the lowest price to show the volatility of the market price. When the channel is narrow, it means that the market volatility is small; otherwise, the channel width indicates that the market volatility is relatively large.

When the price breaks through the upper track of the channel, it is a possible buy signal; conversely, it is a possible sell signal when breaking the lower track.

The calculation methods of the Donchian channel are as follows:

Upper rail = Max (Highest(n), n), the highest value of the highest price of n days

Lower rail = Min (Lowest(n), n), the minimum value of the lowest price of n days

Middle rail = (upper rail + lower rail)/2

Within the framework of multi-factor analysis in the financial sector, this strategy predicts the price trend after the breakthrough based on the validity hypothesis of the momentum factor. Of course, the effectiveness of this factor has indeed been rigorously verified and complemented by the Fama-French three-factor model and is widely used in financial markets.

Of course, we can optimize and use more reasonable trend-breaking indicators.

So, since the momentum factor is a publicly and widely used factor, then why can the Turtle Trading Law stand out from the crowd? The answer is simple. The Turtle Trading Rules define a set of very strict rules for position control and stop-loss. Let's take a look at each one.

- 1. Basic unit of the position N
...
The action of opening a position comes from the generation of a trend breakthrough signal. If the current price breaks through the upper track, it will generate a buy position signal. If the current price falls below the lower track, it will generate a short position signal (the cryptocurrency market is supported by short selling!)

Initial build size = 1 Unit

- 3. When to add positions?

If the holding position is long and the pr