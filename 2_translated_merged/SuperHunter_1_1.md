...
SuperHunter - Intelligent Learning - Multi-Account Consolidated Display Version
...
function printPositionInfo(exchangeInnerArray, totalProfitUSDT, totalProfitRate) {
    ...
    table.rows.push([{
        body: 'This strategy is USDT-based, a mean reversion arbitrage strategy on Binance futures contracts and low-risk auxiliary grids in parallel (BitMEX supports BTC-based positions)',
        colspan: 5
    }]);
    table.rows.push([{
        body: 'The main arbitrage currencies are BTC/USDT and ETH/USDT, with grid coverage over all trading pairs of Binance perpetual futures',
        colspan: 5
    }]);
