...
// Closes positions based on TP/SL conditions
if (strategy.position_size > 0 and strategy.position_avg_price * (1 + longTP) <= close) // Long Take Profit
    strategy.close("Long", comment=botLongClose)
if (strategy.position_size < 0 and strategy.position_avg_price * (1 - longSL) >= close) // Long Stop Loss
    strategy.close("Long", comment=botLongClose)
if (strategy.position_size < 0 and strategy.position_avg_price * (1 - shortTP) <= close) // Short Take Profit
    strategy.close("Short", comment=botShortClose)
if (strategy.position_size > 0 and strategy.position_avg_price * (1 + shortSL) >= close) // Short Stop Loss
    strategy.close("Short", comment=botShortClose)
// }
```

This translation maintains the original code structure, comments, and inputs while translating the human-readable text into English.