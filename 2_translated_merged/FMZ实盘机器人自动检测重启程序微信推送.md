...
1. Open the website: <https://www.pushplus.plus/>
2. Log in with your own WeChat and follow the official account (not advertising)
   ![IMG](https://www.fmz.com/upload/asset/27d5d579fa87c8f486136.png)
3. Click for one-to-one message
   ![IMG](https://www.fmz.com/upload/asset/27d526d1a1f71b1e4fc15.png)
4. Copy the token and fill it in the `token` variable
   ![IMG](https://www.fmz.com/upload/asset/27dd43d36cf1e78165bc7.png)
5. Fill in the `robotId` variable with the number of the real robot you need to monitor (list format)
...
but you need to keep the computer turned on all the time. Alternatively, you can run it on your own server.
Running on a server requires installing third-party libraries in advance**
...
However, you need to keep the computer turned on all the time, or you can run it on your own server.
'''
...
def send_wechat(msg):
    token = '93xxxxxxxxxxxxxxxxxxxxxxxxxxxx57' # Copy the token to this variable
        ...
        if detail['data']['result']['robot']['status'] == 1 and detail['data']['result']['robot']['wd'] == 1:
            ...
            for i in range(4):
                ...
                if robotDetail['data']['result']['robot']['status'] == 1:
                    mess = api('GetRobotLogs', robotId[j], 0, 0, 0, 2, 0, 0, 0, 0, 0, 0, 0)
            ...
            if not status:
                ...
                send_wechat(f"Attempts to restart the real disk {robotId[j]} 4 times failed, please check in time!!\n"
                            f"Attempts 4 times to restart the real disk {robotId[j]} failed, please check in time!!\n"
                            f"Attempts 4 times to restart the real disk {robotId[j]} failed, please check in time!!\n")
            time.sleep(60*10)
```
