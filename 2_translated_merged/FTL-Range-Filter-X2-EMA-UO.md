...
// Ultimate Oscillator
fastLen = input.int(defval=7, title='Fast Length', group=groupUO)
middleLen = input.int(defval=14, title='Middle Length', group=groupUO)
slowLen = input.int(defval=28, title='Slow Length', group=groupUO)
uo = ta.uo(high, low, close, fastLen, middleLen, slowLen)

// Buy Options
buyOverboughtVal = input.float(defval=60.0, title='UO Overbought Value', group=GROUP_BUY_OPTIONS)
showBuyOnlyNotOverbought = input.bool(defval=true, title='Show BUY only when not overbought (UO)', group=GROUP_BUY_OPTIONS)
showBuyAboveEMA = input.bool(defval=true, title='Show BUY only above the EMA', group=GROUP_BUY_OPTIONS)

// Sell Options
sellOversoldVal = input.float(defval=40.0, title='UO Oversold Value', group=GROUP_SELL_OPTIONS)
showSellOnlyNotOversold = input.bool(defval=true, title='Show SELL only when not oversold (UO)', group=GROUP_SELL_OPTIONS)
showSellBelowEMA = input.bool(defval=true, title='Show SELL only below the EMA', group=GROUP_SELL_OPTIONS)

// Indicators
buySignal = uo < buyOverboughtVal and src4 > ema and showBuyAboveEMA ? true : false
sellSignal = uo > sellOversoldVal and src4 < ema and showSellBelowEMA ? true : false
...
plotshape(series=buySignal, style=shape.triangleup, color=color.green, location=location.belowbar)
plotshape(series=sellSignal, style=shape.triangledown, color=color.red, location=location.abovebar)
