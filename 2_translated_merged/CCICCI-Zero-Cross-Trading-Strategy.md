...
The CCI Zero Cross Trading Strategy (CCI Zero Cross Trading Strategy) is a quantitative trading strategy based on the Commodity Channel Index (CCI). The strategy generates trading signals by tracking the crossover situations between the CCI indicator and the zero level. It establishes long positions when the CCI crosses above zero and short positions when the CCI falls below zero, belonging to the trend-following type.
...
2. Monitor the crossover situations between the CCI and the zero level. A buy signal is generated when the CCI crosses zero from below, and a sell signal is generated when the CCI drops below zero from above.
...
``` pinescript
//@version=2
strategy("CCI Zero Cross Strategy", shorttitle="CCI_ZCS", overlay=true)

///////////// CCI
CCIlength = input(20, minval=1, title="CCI Period Length")
cci = cci(close, CCIlength)
plotcci = plot(cross(cci, 0) ? close : na, title="CCI Plot", color=color.blue, linewidth=1)
plotshape(series=cross(cci, 0), title="Buy Signal", location=location.belowbar, color=color.green, style=shape.triangleup, size=size.small)
plotshape(series=cci < -100 ? close : na, title="Short Stop Loss", location=location.belowbar, color=color.red, style=shape.square, size=size.tiny)
...
if (longCondition)
    ...
    strategy.exit("Long Exit", from_entry="Long", stop=-100.0)
    
if (shortCondition)
    strategy.entry("Short", strategy.short)
    strategy.exit("Short Exit", from_entry="Short", stop=100.0)
```

This PineScript defines the CCI Zero Cross Trading Strategy, using the Commodity Channel Index (CCI) to generate trading signals and manage risk through stops.