...
1. Calculate PMax based on ATR and Moving Average. Generate a buy signal when price crosses over PMax.
2. Add T3 indicator and price as filters to ensure entering only during an upward trend.
3. Set take profit methods: first use the double BAND strategy to determine the first take profit; then use the Dice strategy to determine subsequent take profits and stop losses.
4. Use MOST indicator to assist in determining trends, reducing unnecessary reverse operations.

## Advantage Analysis

1. The PMax strategy itself has the advantage of avoiding high-chase stops, making the moving stop mechanism more effective at reducing drawdowns (DD).
2. Double filters ensure that we only enter positions during an upward trend, avoiding false breakouts.
...
3. Can add an auto-adjust take profit mechanism, dynamically adjusting subsequent take profit points based on volatility and return rate.
4. Can test allowing short positions, adjusting position proportions based on filters.

## Summary

The overall strategy is centered around using PMax for entry judgment, with multiple layers of filtering and take profit methods to optimize performance in trending products. After simplifying the filters, optimizing take profit settings, and appropriately adjusting position management, the strategy can be optimized to suit more products, achieving better results in live trading.
...
| v_input_12 | false | Show Price/Pmax Cross Signals? |
...
| v_input_18 | 5 | Fibonacci T3 Length |
...
| v_input_24 | 25 | Snake Width |
| v_input_25 | 20 | Dominant Cycle Width |
| v_input_26 | 8 | Fast Avg |
| v_input_27 | 26 | Slow Avg |
| v_input_28 | 0.8 | Width |
| v_input_29 | false | TP Filter avg2/avg4? |
| v_input_30 | false | Show TP Level and Exit Level Count On/Off? |
