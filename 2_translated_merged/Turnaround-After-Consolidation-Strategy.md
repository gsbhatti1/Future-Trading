...
1. The strategy uses the Stochastic oscillator indicator to determine if the stock price has entered consolidation. Oscillation of the Stochastic oscillator in overbought or oversold zones indicates price consolidation.
2. When the Stochastic oscillator oscillates, candlestick directional changes are used to detect trend reversal points. A candle change from black to white signals consolidation end and go long. A change from white to black signals consolidation end and go short.
3. Profit targets and stop loss after entry are set dynamically based on entry price using trailing stops. Fixed profit/loss is used for full position, while trailing stops are used for partial positions.
4. The strategy supports both full and partial position trading. Fixed points are used for full position, while trailing stops are used for partial positions.
...
The turnaround after consolidation strategy identifies short-term consolidation using the Stochastic oscillator and trades at trend reversal points after the consolidation phase. It has a decent winning rate and allows locking in segment profits in trends. However, Stochastic is prone to false signals. Accuracy can be improved by optimizing parameters, adding filters, etc. In addition, optimizing trailing stops, controlling position sizing, and avoiding event risks are areas that require focus. Overall, this strategy provides a reference model but needs tuning and risk control for live trading based on individual trading style.
...
|v_input_int_6|10|Start Operation Time - Hour|
|v_input_int_7|40|End All Trades Minute|
|v_input_int_8|17|Close Time - Hour|
|v_input_int_9|50|End New Trades Minute|
|v_input_int_10|50|End All Trades Minute|
|v_input_int_11|90|Stochastic Target - For Short|
|v_input_int_12|10|Stochastic Target - For Buy |
|v_input_1|true|Partial ? |
|v_input_int_13|150|Profit Points |
|v_input_int_14|150|Loss Points|
|v_input_int_15|50|Partial Points |
