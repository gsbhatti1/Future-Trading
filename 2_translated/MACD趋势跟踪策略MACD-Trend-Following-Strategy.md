...
4. When a golden cross happens, record the current closing price as the long entry point, set a stop loss point, and open a long position.
5. When a death cross happens, record the current closing price as the short entry point, set a stop loss point, and open a short position.
6. When holding a long position, if the profit ratio reaches the preset target or drawdown reaches the stop loss point, close the position to realize profits.
7. When holding a short position, if the profit ratio reaches the preset target or drawdown reaches the stop loss point, close the position to realize profits.

Through this trend following mechanism, the strategy can timely capture market trend turns and make profits.

## Advantage Analysis

The MACD Trend Following Strategy has the following advantages:

1. The source of strategy signals is singular and clear, generated directly by the MACD indicator, avoiding interference.
2. Utilize the golden cross and death cross characteristics of the MACD indicator to determine market trend directions with accurate judgments.
3. Timely tracking of trend turns with strong profit tracking capability.
4. Proper risk control in place, including a stop loss mechanism.
...
3. Balancing between profit tracking ratios and stop loss points is difficult, posing a risk of overtracking leading to losses.
...
3. Optimize parameters for profit tracking ratios and stop loss points.
...
3. Set up a dynamic trailing stop loss mechanism. Stop loss points can be adjusted dynamically based on volatility.
4. Optimize the signal determination logic for opening positions. More rigorous trigger conditions can be set.
5. Incorporate machine learning models to filter out signals. Models can be trained to judge the reliability of signals.

## Conclusion

In general, the MACD Trend Following Strategy is a relatively mature quantitative strategy. It utilizes the MACD indicator to determine market trend directions and controls risks with stop loss mechanisms, which can effectively track price trends. However, the MACD indicator itself also has some flaws, such as generating false signals. Therefore, there are still rooms for further optimization of this strategy, mainly focusing on aspects like indicator parameters, stop loss mechanisms, and signal filtering.
...
if ta.crossover(macdLine, signalLine) and macdLine < highestMACD and macdLine > 0 and haveOpenedLong == false
    ...
    stoploss := stoploss + minProfit * close
...
// Exit long position when profit reaches target or drawdown exceeds stop loss
if not na(entryLongPrice) and (strategy.equity / entryLongPrice - 1 >= minProfit or ta.rsi(close, 14) <= 30)
    strategy.close("Long")

// Exit short position when profit reaches target or drawdown exceeds stop loss
if not na(entryShortPrice) and (strategy.equity / entryShortPrice - 1 <= -minProfit or ta.rsi(close, 14) >= 70)
