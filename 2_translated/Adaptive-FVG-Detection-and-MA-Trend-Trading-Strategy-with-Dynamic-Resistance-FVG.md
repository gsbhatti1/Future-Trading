...
// Check for FVG in the selected timeframe
...
// Display FVG zones on the graph
if (fvgFilled)
    var box fvgBox = na
    if (na(fvgBox))
        fvgBox := box.new(left=bar_index[1], top=fvgHigh, bottom=fvgLow, right=bar_index, color=color.green, border_width=2)
```
[/trans]