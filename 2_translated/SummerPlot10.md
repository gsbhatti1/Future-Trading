...
function setChart(title, line, col, is_single, series_type, is_stock) {
    ...
    if (char) {
        ...
        _Cfgs.push({
            ...
            //#region Setting Time Range Selectable
            rangeSelector: {
                buttons: [{
                    type: 'day',
                    count: 1,
...
function getSeriesIndex(title, line, series_type) {
    ...
    for (let i = 0; i < _Cfgs.length; i++) {
        for (let j = 0; j < _Cfgs[i].series.length; j++) {
            ...
            if (_Cfgs[i].title.text === title) {
                cfg_index = i;                              // Record the index of the corresponding chart
    ...
    // If there is no corresponding series
...
function main() {
    ...
    while (true) {
        ...
        $.PlotLineSummer('Order Book Chart', 'Buy 1', ticker.Buy, time, 12, true);
        $.PlotLineSummer('Order Book Chart', 'Sell 1', ticker.Sell, time, 12, true);
        $.PlotBarSummer('Difference Chart', 'Diff 1', diff, time, 12, true);
        $.PlotBarSummer('Difference Chart', 'Diff 2', diff * 2, time, 12, true);
        $.PlotLineSummer('Order Book Chart2', 'Buy 1', ticker.Buy, time, 8, true);
        $.PlotLineSummer('Order Book Chart2', 'Sell 1', ticker.Sell, time, 8, true);
        $.PlotLineSummer('Order Book Chart3', 'Buy 1', ticker.Buy, time, 4, true);
        $.PlotLineSummer('Order Book Chart3', 'Sell 1', ticker.Sell, time, 4, true);
        let data = [["A", 25 + (++count)],
                    ["B", 25 - count],
                    ["C", 25],
                    ["D", 25]];
