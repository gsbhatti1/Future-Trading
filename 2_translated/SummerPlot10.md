...
1、Initial version.
----------- 2023.3.16 --------------
1、Adjusted the time range selector.
*/
//#endregion
...
function setChart(title, line, col, is_single, series_type, is_stock) {
    ...
    if (char) {
        ...
        _Cfgs.push({
            ...
            legend: {
                enabled: true,                                      // Series user selectable
            },
            extension: {
            ...
            //#region Set Time Range Selection Available
            rangeSelector: {
                buttons: [{
                    type: 'day',
                    count: 1,
    ...
    for (let i = 0; i < _Cfgs.length; i++) {
        for (let j = 0; j < _Cfgs[i].series.length; j++) {
            ...
            if (_Cfgs[i].title.text === title) {
                ...
                last_series_index = index;                  // Record the corresponding chart's last index
                if (_Cfgs[i].series[j].name === line && _Cfgs[i].series[j].type === series_type) {
                    // Find the corresponding series, return its index directly
                    return index;
                }
            }
        }
    }
    // If no corresponding series exists
...
function main() {
    ...
    while (true) {
        ...
        $.PlotLineSummer('Order Book Chart', 'Bid 1', ticker.Buy, time, 12, true);
        $.PlotLineSummer('Order Book Chart', 'Ask 1', ticker.Sell, time, 12, true);
        $.PlotBarSummer('Difference Chart', 'Difference 1', diff, time, 12, true);
        $.PlotBarSummer('Difference Chart', 'Difference 2', diff * 2, time, 12, true);
        $.PlotLineSummer('Order Book Chart 2', 'Bid 1', ticker.Buy, time, 8, true);
        $.PlotLineSummer('Order Book Chart 2', 'Ask 1', ticker.Sell, time, 8, true);
        $.PlotLineSummer('Order Book Chart 3', 'Bid 1', ticker.Buy, time, 4, true);
        $.PlotLineSummer('Order Book Chart 3', 'Ask 1', ticker.Sell, time, 4, true);
        let data = [["A", 25 + (++count)],
                    ["B", 25 - count],
                    ["C", 25],
                    ["D", 25]];
        $.PlotPieSummer('Pie Chart 1', 'Pie Chart 1', data, 6, true);
        $.PlotPieSummer('Pie Chart 2', 'Pie Chart 2', data, 6, true);
    }
}
```
