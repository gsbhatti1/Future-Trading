...
// —————— Risk Management ——————
maxDrawdown = 2 * stopLossPerc // Maximum allowed drawdown as a percentage of the stop loss

trailStopPrice = na
if (longCondition and ta.lowest(low, maxDrawdown / stopLossPerc) < close)
    trailStopPrice := close - (maxDrawdown / 100) * close
stopLossLevel = trailStopPrice + (stopLossPerc / 100) * close

// —————— Exit Conditions ——————
if (strategy.opentrades > 0 and ta.closeabove(stopLossLevel, 1))
    strategy.exit("Long Stop Loss", "Long")
    
if (strategy.opentrades > 0 and ta.closebelow(stopLossLevel, 1))
    strategy.exit("Short Stop Loss", "Short")

// —————— Visualization ——————
plot(emaFast, color=color.blue)
plot(emaSlow, color=color.red)
hline(50, "RSI Level 50", color=color.gray)
fill(emaFast, emaSlow, color=color.green, transp=90)

```

This completes the translation of the trading strategy document.