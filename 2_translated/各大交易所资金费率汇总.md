...
function UpdateStatus(){
    let table = { 
        type: 'table', 
        title: 'Funding Rate%', 
        cols: ['symbol'], // initialize columns with symbol
        rows: [] 
    };

    let fundingRates = {};

    // directly construct the rate information from existing fundings data
    exchanges.forEach((e) => {
        let eName = e.GetName();
        ...
        if (fundings) {
            for (let symbol in fundings) {
                let rate = fundings[symbol].Rate; // assume each returned object has a Rate property
                ...
                fundingRates[symbol].exchanges[eName] = rate; // record the rate
            }
        }
    });

    // construct each trading pair's row and average rate
    for (let symbol in fundingRates) {
        let data = fundingRates[symbol];
        let averageRate = data.total / data.count; // calculate the average rate

        // add column headers to the table only once
        if (table.cols.length === 1) { // add trading pair columns only on first pass
            for (let e of exchanges) {
                table.cols.push(e.GetName());
            }
            table.cols.push('Average Rate'); // add average rate column
        }

        // construct row data
        let row = [symbol];
        for (let e of exchanges) {
            row.push(data.exchanges[e.GetName()] || 0); // fill the rates from each exchange
        }
        row.push(averageRate); // fill the average rate
