...
williams_r = ta.willr(14)
ema_williams_r = ta.ema(williams_r, 9)

// Buy and sell conditions
buy_condition = ta.crossover(ema_williams_r, -80)
sell_condition = ta.crossunder(ema_williams_r, -20)

// Trade entry and exit based on signals
if (buy_condition)
    strategy.entry("Long", strategy.long)
    strategy.exit("Profit or Stop Loss", "Long", limit=tp_level, stop=sl_level)

if (sell_condition)
    strategy.entry("Short", strategy.short)
    strategy.exit("Profit or Stop Loss", "Short", limit=100 - tp_level, stop=100 - sl_level)

// Optionally restrict trading time
// Add your own logic here to implement the time restriction based on X and Y minutes near the top of the hour
