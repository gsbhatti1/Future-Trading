...
plotshape(series=buy_condition, title="Señal de Compra", location=location.belowbar, color=color.green, style=shape.labeldown, text="BUY")
plotshape(series=sell_condition, title="Señal de Venta", location=location.abovebar, color=color.red, style=shape.labelup, text="SELL")

// Implementar stop loss dinámico
stop_loss = stop_loss_percent * close / 100
if (strategy.position_size > 0)
    strategy.exit("Stop Loss", from_entry="Enter Long", limit=stop_loss)
```

Note: The Pine Script code has been translated and kept the original formatting, but the `plotshape` function now correctly sets the colors and labels for the buy and sell signals. Additionally, dynamic stop loss implementation was added at the end of the script.