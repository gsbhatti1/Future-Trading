...
2. Monitor the crossover situations between the CCI and the zero line. A buy signal is generated when the CCI crosses zero from below. A sell signal is generated when the CCI drops below zero from above.
...
cci = ta.cci(v_input_1)
plot(cross(cross(cci, 0), cci > -100) ? v_input_1 : na, color=color.green, style=plot.style_linebr, linewidth=2)
plot(cross(cross(cci, 0), cci < 100) ? v_input_1 : na, color=color.red, style=plot.style_linebr, linewidth=2)

// Long Entry
if (cross(cross(cci, 0), ccci > -100))
    strategy.entry("Long", strategy.long)
    
// Short Entry
if (cross(cross(cci, 0), cci < 100))
    strategy.entry("Short", strategy.short)

// Stop Loss
strategy.exit("Stop Loss Long", "Long", stop=-100)
strategy.exit("Stop Loss Short", "Short", stop=100)
```