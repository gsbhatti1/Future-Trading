...
// however dual thrust uses the historical price to construct and update the look back period - 
...
// Then take the two larger ones, multiply by k values. The results are called trigger values.

// On the second day, the opening price is recorded, 
// and then immediately after the price exceeds (opening + trigger value), 
// or the price is lower than (opening - trigger value), short selling immediately.
...
// K1 should be smaller than k2 if you are bullish on the market and k1 should be much bigger if you are bearish on the market.
