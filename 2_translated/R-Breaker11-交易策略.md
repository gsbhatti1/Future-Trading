...
# Get current market value of assets
def GetStockcap():
    account=GetAccount()
    ticker = GetTicker()
...
def print_log(k_p,data=""):  # Output
    try:
        ...
        msg_data0=("This session started at: %s Duration: %s\r\n"%(start_time,Caltime(start_time,date1)))
        #################################################
        msg_data1=("Initialization status: %s\r\n Current state: %s\r\n"%(beng_Account,end_Account))
        #################################################
        end_Balance=(end_Account.Stocks*end_ticker)+end_Account.Balance # Current balance on the books
        msg_data2=("Initial money: %s Now money: %s Profit and loss: %s\r\n"%(str(beng_Balance),str(end_Balance),str(end_Balance-beng_Balance)))
        #################################################
        total = end_Account.Balance+end_Account.Stocks*_C(exchange.GetTicker).Last # Total account
        ...
        msg_data4=("This session's profit: %s(RMB)\tTotal Profit & Loss:%.2f(RMB) %.2f\r\n"%(str(gains),income,roi))
        #################################################
        # Calculation method of gains
        # Gains calculation  Floating profits: (current coins - initial coins) * current price + (current money - initial money)
