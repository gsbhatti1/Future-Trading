...
A trading strategy based on longing resistance breakout and shorting support breakout. It identifies highs and lows using fractals with a confirmation of two bars, resulting in a two-bar lag. The difference between the simple moving average (SMA) of highs and the SMA of lows is used as an alternative support-resistance level. This idea was taken from synapticEx's indicator Nebula-Advanced-Dynamic-Support-Resistance. Position entry occurs on the breakout defined by fractals, while position exit is triggered when there is a bar change in the opposite direction to the position with a magnitude greater than the SMA difference.
