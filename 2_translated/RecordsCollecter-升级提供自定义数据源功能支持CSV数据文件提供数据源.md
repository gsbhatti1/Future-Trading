...
RecordsCollecter-Enhanced with Custom Data Source Functionality Supporting CSV Files as Data Sources
...
Related Article: https://www.fmz.com/bbs-topic/5669
...
|isOnlySupportCSV|false|Only provide CSV data source|
|filePathForCSV||Path to the CSV file|
...
class Provider(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        try:
            ...
            # Currently, the backtesting system can only select exchange names from a list. When adding custom data sources, set it to Binance.
            ...
            if isOnlySupportCSV:
                # Process CSV reading, filePathForCSV path
                listDataSequence = []
            ...
            Log("Connecting to the database service, retrieving data. Database: ", exName, " Table: ", tabName)
            ...
            Log("Query conditions: ", dbQuery, " Number of query results: ", exRecords.find(dbQuery).count(), " Total number of database entries: ", exRecords.find().count())
...
def main():
    ...
    if (isOnlySupportCSV):
        try:
            # _thread.start_new_thread(createServer, (("localhost", 9090), ))         # Testing on local machine
            _thread.start_new_thread(createServer, (("0.0.0.0", 9090), ))         # Testing on VPS server
            Log("Starting custom data source service thread, data provided by CSV files.", "#FF0000")
        except BaseException as e:
    ...
    # Connect to the database service, service address mongodb://127.0.0.1:27017 as per server configuration
