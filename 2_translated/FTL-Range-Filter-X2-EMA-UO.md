...
// Add buy and sell signals based on EMA crossing and Ultimate Oscillator conditions

// UO Fast Length
fastlen = input.int(7, minval=1, title='Fast Length', group=groupUO)
// UO Middle Length
middlenum = input.int(14, minval=1, title='Middle Length', group=groupUO)
// UO Slow Length
slownum = input.int(28, minval=1, title='Slow Length', group=groupUO)

// Ultimate Oscillator
uo = ta.uo(src4, fastlen, middlenum, slownum)

// Buy Options
buynotoverbought = input.bool(true, title='Show BUY only when not overbought (UO)', group=GROUP_BUY_OPTIONS)
buyaboveema = input.bool(true, title='Show BUY only above the EMA', group=GROUP_BUY_OPTIONS)
uobuyvalue = input.float(60.0, minval=1, maxval=100, title='UO Overbought value', group=GROUP_BUY_OPTIONS)

// Sell Options
sellnotoversold = input.bool(true, title='Show SELL only when not oversold (UO)', group=GROUP_SELL_OPTIONS)
sellobelowema = input.bool(true, title='Show SELL only below the EMA', group=GROUP_SELL_OPTIONS)
uosellvalue = input.float(40.0, minval=1, maxval=100, title='UO Oversold value', group=GROUP_SELL_OPTIONS)

// Buy and Sell Conditions
buycondition = not uo > uobuyvalue and ema > src4

sellcondition = not uo < uosellvalue and ema < src4

// Alerts
alertcondition(buycondition, title='Buy Signal')
alertcondition(sellcondition, title='Sell Signal')
