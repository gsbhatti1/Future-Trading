...
3. In addition, when the buy signal triggers, the stock price must be within 10% range of the two-year high, not too far away. It also needs to be below the 13-day line and above the 21-day line to ensure proper timing.
4. For open positions, if the price breaks 5% below the 21-day moving average (MA) line or declines 20% from the two-year high, the position will be stopped out to lock in profits.
...
2. The 13-day exponential moving average (EMA) serves as the entry filter to avoid whipsaws and determine stronger momentum.
...
3. The 13-day line may not filter out consolidations well, generating excessive false signals. Can consider extending to the 21-day line.
...
### Conclusion
In summary, this is a unique long-term breakout strategy with the key being the two-year high price level and the 13-day EMA line serving as an entry filter. It has certain advantages but also room for improvements, worth further research and exploration.
...
// This script accepts from and to date parameters for backtesting.
...
// Plot a green triangle up whenever it's a new high.
