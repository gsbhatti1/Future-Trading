...
It can be interesting to see if a top or bottom is associated with a specific candle pattern; this allows us to study the potential of such a pattern to indicate a reversal. You can hover over a label with a specific pattern to see more details about it.

Note that the labels are offset and will appear later in real-time, so this indicator is not intended to detect tops/bottoms in real-time.
