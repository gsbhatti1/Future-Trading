...
function WSConnecter_Huobi (method, symbol) {
    ...
    */
    ...
    if (method == "GetTicker") {
        ...
        throw "Unimplemented: " + method + " method!"
    }
    
    var self = {}
    self.method = method
    self.pingCount = 0
    Log("Dial function link:", "wss://api.huobi.pro/ws|compress=gzip&mode=recv&reconnect=true&payload=" + strSubscribe)
    Log("Subscription info:", strSubscribe)
    ...
    self.Read = function () {
        try {
            ...
            if (objRet.ping) {
                ...
                // Log("huobiConn received ping, sending pong:", JSON.stringify(pong))                
                self.client.write(JSON.stringify(pong))
                self.pingCount++
                return null
            } else {
                /* Wrap into FMZ ticker structure
                {
    ...
    self.Close = function () {
        self.client.close()
        Log("method:", self.method, "closed ws connection")
    }
...
function WSConnecter_Binance (method, symbol) {
    ...
    self.pingCount = 0
    self.method = method
    Log("Dial function link:", "wss://stream.binance.com:9443" + strSubscribe + "|reconnect=true")
    ...
    self.Read = function () {
        try {
            ...
            if (objRet.ping) {
                var pong = {"pong" : objRet.ping}
                Log("binanceConn received ping, sending pong:", JSON.stringify(pong))                
                self.client.write(JSON.stringify(pong))
                self.pingCount++
                return null
            } else {
                ...
                */
                ...
                Log("Pushed ticker:", ticker)   // Test log
...
var _DictConnectCreater = {
    ...
    "Binance" : WSConnecter_Binace,
}
```

Please note that there was a small typo in the last line where `WSConnecter_Binance` should be used instead of `WSConnecter_Binace`.