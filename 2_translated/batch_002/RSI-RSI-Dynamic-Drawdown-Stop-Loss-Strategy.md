...
2. The dynamic drawdown stop-loss mechanism effectively controls the strategy's maximum drawdown, reducing overall risk.
...
if (accumulation and drawdown < max_drawdown or distribution and drawdown < max_drawdown)
    strategy.entry("Buy", strategy.long)
    if not na(strategy.opentradesCLOSED_POSITIONS-entry.price) and (distribution or drawdown >= max_drawdown)
        strategy.close("Buy")
    
    if not na(strategy.opentradesCLOSED_POSITIONS-entry.price) and (accumulation or drawdown >= max_drawdown)
        strategy.close("Buy")
```
[/trans]