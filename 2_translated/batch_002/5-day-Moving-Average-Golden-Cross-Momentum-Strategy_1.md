...
This strategy uses the crossover of 5-day and 78-day moving averages to generate momentum chasing signals, aiming to capture short-term price breakouts.
...
3. When the 3-day line is above the 78-day line, and the 78-day line is above the 195-day line, it indicates an uptrend channel, which also triggers a buy signal.
4. Set a 6ATR dynamic profit-taking line; sell when the price falls below this line.
5. A sell signal is triggered when the 3-day line crosses back below the 195-day line.

## Advantages

1. Multiple MA crossovers effectively filter false breakouts.
2. Dynamic profit-taking avoids whipsaws.
3. Backtesting shows an average holding period of only 2 hours per trade, suitable for short-term momentum trading.
4. Maximum drawdown is controlled around 20%.

## Risks

1. Fixed MA parameters fail to adapt to changing markets.
2. The 1-year sample period is limited; a larger dataset is needed to verify the strategy.
3. Profit-taking and stop-loss parameters need optimization for risk control.
4. It fails to adapt to price gaps.
5. High transaction costs are likely.

## Enhancements

1. Test different MA combinations for optimization.
2. Optimize profit-taking and stop-loss rules for a better risk-return balance.
3. Set entry filters to reduce the probability of being trapped.
4. Optimize position sizing, adding positions gradually based on strength.
5. Test across different products and longer timeframes.
6. Conduct Monte Carlo simulations to evaluate maximum drawdown.

## Summary

This strategy identifies an uptrend using MA crossovers and sets dynamic profit and stop rules with good backtest results. However, the limited sample period and parameter stability need further verification, as well as handling gaps in prices. Further backtesting over larger datasets, adding more filters to reduce false signals, optimizing profit and stop parameters, and evaluating transaction costs are required. If it passes comprehensive optimization and verification tests, this strategy can become a robust short-term momentum chasing system.
...
// Momentum trading Strategy: When S&P 500 index is at up trend (or above 60 SMA), buy 10+ stocks in the top 20% stock RS ranking using this MA5X_L strategy. Change stocks when any stock exits by algorithm.  
// Back test start since 2020/7/1, each long entry for condition 1 is $30,000, and condition 2 is $20,000 with a maximum of 2 long positions.
// Setup: 10-minute chart
// Buy Condition 1) 3 WMA crosses up 180 WMA (5-day) 2) 3WMA > 60WMA > 180WMA - Uptrend Arrangement (UTA)
// Exit Condition 1) 3 WMA crosses under 180 WMA 2) Position profit > 20% and 3 WMA crosses under 6 ATRs line (green)
//@version=4
...
// Long Entry Condition 1 : s_ma crosses up l_ma
...
// Long Exit Condition 1 : s_ma crosses under l_ma
