...
The fill option is added between the moving averages (it is deactivated by default, activate in configuration if you wish), if both averages are green the fill will be green, if both averages are red the fill will be red, this would confirm a trend healthy when using a fast and slow average. In the case that the fast average changes direction it will be filled in purple color, this can indicate several things: a correction of the trend is being generated, possible rest of the price, possible start of laterality, possible change of trend. When the color is purple the trader will have to analyze the general context to be able to define what could be happening.
