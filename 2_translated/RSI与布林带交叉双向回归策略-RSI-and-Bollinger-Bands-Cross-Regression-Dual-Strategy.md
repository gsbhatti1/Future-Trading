...
5. The strategy employs an OCA (One-Cancels-All) order management mechanism to ensure only one active trade at a time.
...
//@version=5
strategy("RSI with Bollinger Bands Dual Strategy (by ChartArt) v2.2", shorttitle="CA_RSI_BB_Strategy_2.2", overlay=true)

// RSI and Bollinger Bands dual strategy by ChartArt - Simplified Version
//
// Chinese version 3, BY Henry
// Original creation from ChartArt, January 18, 2015
// Updated to Pine Script v5 version, removed background color, candlestick colors, and plot of P&L features
//
// Strategy Description:
// The strategy combines the use of RSI and Bollinger Bands.
// It sells when price is above the upper track and RSI is overbought,
// and buys when price is below the lower track and RSI is oversold.
//
// This strategy only triggers based on dual conditions
// where both RSI and Bollinger Bands are in an overbought or oversold state.
