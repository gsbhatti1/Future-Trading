...
1. Acts like a dead indicator like its ancestor Magic Trend in sideways market conditions and doesn't give many false signals.
2. With another line 2 bars offsetted from the original one, Alpha Trend has BUY and SELL signals from their crossovers.
...
3. Alpha Trend lines:
   ...
   - and acting as trailing stop losses. The more Alpha Trend lines are straighter, the stronger supports and resistances become.

4. Trend Magic has CCI in calculation; Alpha Trend has MFI as momentum but when there is no volume data, MFI has 0 values, so there is a button to change calculation considering RSI after checking the relevant box to overcome this problem when there is no volume data on that chart.
