...
The Noro Shifted Moving Average Stop Loss Strategy is a trend following strategy. It calculates the 3-day simple moving average line, and sets an entry level above it and a stop loss below it at given percentages. Take profit levels are also set. This allows opening positions when trends start, and stopping out when trends reverse.

## Strategy Logic

The core of this strategy is calculating the 3-day simple moving average line `ma`. Then a percentage `lo` is added above `ma` to get the entry level `long`. When price crosses above `long`, long positions are opened. Below `ma`, a percentage `sl` is subtracted to get the stop loss level `stop`. When price drops below `stop`, positions are stopped out. There are also take profit levels `take` set at a percentage `tp` above the current average holding price.

The specific rules are:

1. Calculate 3-day simple moving average `ma`
2. Entry level `long` = `ma + ma * lo%`
3. Take profit level `take` = Current average holding price + Current average holding price * `tp%`
4. Stop loss level `stop` = Current average holding price - Current average holding price * `sl%`
...
[Detail](https://www.fmz.com/strategy/440438)

---

Last Modified: 2024-01-30 15:49:34