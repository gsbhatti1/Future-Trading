```markdown
Name: RSI-and-BBand-simultaneously-OverSold

Author: ChaoZhang

Strategy Description:
This indicator is created by combining the standard period RSI indicator with an Oversold limit of 32, an Overbought limit of 70 and a period of 14 (these values can be changed optionally from the entries and still tabs of the indicator settings) and the Bollinger Band indicator with a standard deviation of 2 and a period of 20. Also, the RSI Oversold is indicated by an upward green triangle where the price simultaneously falls below the BB and the lower limit (Low) (i.e. below 32), while the RSI Overbought (i.e. above 70) at the same time the price rises above the BB and the upper limit (Upper) is indicated by a downward red triangle. An alarm condition is established on these conditions.

**Backtest**
![](https://www.fmz.com/upload/asset/1f1cc8bca66e3e9a1fd.png)
...
| v_input_3       | 32       | Enter RSI Bottom Value         |
...
// RSI hesapla ve kaynak, period seçimi için seçki kutusu ve değer giriş kutuları burada yapıldı
...
// Bollinger Band hesapla ve kaynak, period seçimi için seçki kutusu ve değer giriş kutuları burada yapıldı
...
alertcondition(bullishkosulu, title="Bullish koşulu üzerine alarm kur", message="{{interval}} - RSI<32 ve Fiyat<BB Lower - {{ticker}} - En düşük fiyat:{{low}} - {{timenow}}")
