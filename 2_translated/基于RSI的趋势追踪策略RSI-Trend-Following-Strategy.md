...
// etoroStopTicks: Set it accordingly to the stock (to corresponds to eToro default of 50% for example...)
...
// 120 because it is approximately the number of ticks for default SL of 50% at x5 leverage for copper (no fee)...
price = close

vrsi = rsi(price, length)

// === ENTRY LOGIC ===
if vrsi < overSold and crossover(vrsi, overSold)
    strategy.entry("Buy", strategy.long)

if vrsi > overBought and crossunder(vrsi, overBought)
    strategy.entry("Sell", strategy.short)

stopLoss = etoroStopTicks * point_size
if isLong
    strategy.exit("SL Long", from_entry="Buy", stop=stopLoss)
else if isShort
    strategy.exit("SL Short", from_entry="Sell", stop=stopLoss)
```

This completes the translation and retains all formatting as specified.