...
if close <= (low1 * (1 - entry_factor/100)) and strategy.position_size == 0
    strategy.entry(id = "short_entry", long = false, qty = 1)
    
strategy.close(id = "short_entry", when = strategy.position_size != 0 and (close > low1 * (1 + entry_factor/100) or close < low1 * (1 - (entry_factor + profit_target)/100)))
```
```