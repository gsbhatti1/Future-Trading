...
longCondition = ta.crossover(ema110, ema200)  // EMA110 crosses above EMA200, go long
shortCondition = ta.crossunder(ema110, ema200)  // EMA110 crosses below EMA200, go short
...
if (longCondition and not isLong)  // If long condition met and no existing long position
    if (isShort)  // If current position is short, exit from short first
...
if (shortCondition and not isShort)  // If short condition met and no existing short position
    if (isLong)  // If current position is long, exit from long first
        strategy.close("Long")
    strategy.entry("Short", strategy.short)  // Enter short
