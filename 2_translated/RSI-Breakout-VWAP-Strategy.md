...
In short, the RSI moving average breakthrough VWAP strategy combines a variety of indicators to identify trading opportunities, but it requires careful testing and parameter adjustment to adapt to different market environments. Only by controlling risks can this strategy be used in the long term.
[/trans]

This strategy applies the RSI indicator on VWAP, and determines the long/short direction based on RSI threshold breakouts. Specifically, it goes short when RSI breaks above the overbought level, and goes long when RSI breaks below the oversold level. It also forces exit after a certain period of consecutive threshold breaks.
