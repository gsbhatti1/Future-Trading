...
upPlot = plot(trend == 1 ? up : na, title='Uptrend (Fast)', style=plot.style_linebr, linewidth=2, color=color.new(color.green, transp_st))
buySignal = trend == 1 and trend[1] == -1
plotshape(buySignal ? up : na, title='Uptrend Start (Fast)', location=location.absolute, style=shape.circle, size=size.tiny, color=color.new(color.green, transp_st))
...
dnPlot = plot(trend == 1 ? na : dn, title='Downtrend (Fast)', style=plot.style_linebr, linewidth=2, color=color.new(color.red, transp_st))
sellSignal = trend == -1 and trend[1] == 1
plotshape(sellSignal ? dn : na, title='Downtrend Start (Fast)', location=location.absolute, style=shape.circle, size=size.tiny, color=color.new(color.red, transp_st))
...
fill(mPlot, upPlot, title='Uptrend Shading (Fast)', color=color.new(longFillColor, transp_hl))
fill(mPlot, dnPlot, title='Downtrend Shading (Fast)', color=color.new(shortFillColor, transp_hl))
