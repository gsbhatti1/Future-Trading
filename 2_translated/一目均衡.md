...
One-Line Balance Basic Edition

Based on the hourly chart, it predicts for one to two months. Buy or sell signals are generated using the cloud thickness, baseline, and conversion lines without setting a stop-loss.
...
|---|---|---|
|keh|12|HullMA|
|TenkanSenPeriods|9|TenkanSen Conversion Value Interval|
|KijunSenPeriods|24|Kijun Sen Baseline Interval|
|SenkouSpanBPeriods|51|Senkou Span B Leading Upper Limit 2 Interval|
|displacement|24|Displacement Delay Line Interval|
...
function onTick(exchange) {
   ...
   var ttime = records[records.length - 1].Time;
    // Conversion value = (today's highest price of the last 9 days + today's lowest price of the last 9 days) / 2
    var TenkanSen = donchian(records, TenkanSenPeriods);
    TenkanSenArray.push(TenkanSen);

    // Baseline value = (today's highest price of the last 26 days + today's lowest price of the last 26 days) / 2
    var KijunSen = donchian(records, KijunSenPeriods);
    KijunSenArray.push(KijunSen);

    // Leading upper limit 1 (Span A) = (Conversion value + Baseline value) / 2
    var SenkouSpanA = avg(TenkanSen, KijunSen);
    SenkouSpanAArray.push(SenkouSpanA);

    // Leading upper limit 2 (Span B) = (highest price of the last 52 days + lowest price of the last 52 days) / 2
    ...
    // Cloud is formed by Span A and Span B
    ...
    // Delay line (Chinkou Span) = today's closing price as the delay line for 26 days ago

    if(typeof(records[records.length - displacement])!="undefined"){
      var ChikouSpan = records[records.length - displacement].Close;
      $.PlotLine("ChikouSpan(Delay Line)", ChikouSpan, ttime, "green");
    }
    ...
    $.PlotLine("TenkanSen(Conversion Value)", TenkanSen, ttime, "red");
    ...
    if (cross(TenkanSenArray, closeArray) && cross(KijunSenArray, closeArray)) {
        ...
        if (absx < CX) {
            if (close > TenkanSen) {
                exchange.Buy(price, 1, "Buy1");
            } else {
                exchange.Sell(price, 1, "Sell1");
            }
        } else {
            if (close > TenkanSen) {
                exchange.Buy(price, 1, "Buy2");
            } else {
                exchange.Sell(price, 1, "Sell2");
            }
        }
