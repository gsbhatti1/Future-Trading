...
Where `wvf` is price volatility, `sDev` is standard deviation, `midLine` is the average line, `lowerBand` and `upperBand` are the lower and upper limit lines. When the price exceeds the upper limit line, it is considered anomalous volatility.
...
Long entry: When the price exceeds the upper limit or volatility exceeds a threshold, and RSI is below a value, go long.

Short entry: When the price exceeds the upper limit or volatility exceeds a threshold, and RSI exceeds a value, go short.

Exit: Close position when the candlestick body direction is opposite to the position direction.
