...
- At the end of the day, calculate two values: Highest price - Closing price, and Closing price - Lowest price. Then take the larger of these two values, multiply by k value, the result is called the trigger value.
- On the next opening day, record the opening price, then immediately buy when the price exceeds (opening + trigger value), or sell short when the price falls below (opening - trigger value).
- This system is a reversal system without individual stop-loss. That means, reverse signals are also liquidation signals.
...
For more details of the strategy: [http://xueqiu.com/5256769224/32429363](http://xueqiu.com/5256769224/32429363)
...
|AmountOP|true|Number of Contracts to Open Position|
|Interval|2000|Retry Interval (milliseconds)|
|LoopInterval|3|Polling Interval (seconds)|
|PeriodShow|500|Maximum Number of K-line Columns for Chart Display|
...
var ChartCfg = {
    __isStock: true,
    title: {
        text: 'Dual Thrust Upper and Lower Tracks'
    },
    yAxis: {
        plotLines: [{
            value: 0,
...
function Trade(currentState, nextState) {
    ...
    while (true) {
        ...
        if (pos[1] >= AmountOP) {
            Log("Average Entry Price", pos[0], "Quantity:", pos[1]);
            break;
        }
        // pfn(AmountOP - pos[1]);
...
function onTick(exchange) {
    ...
    LogStatus("Price:", Bar.Close, "Upper Track:", UpTrack, "Lower Track:", DownTrack, "Wins: ", Counter.w, "Losses:", Counter.l, "Date:", new Date());
    var msg;
    if (State === STATE_IDLE || State === STATE_SHORT) {
        if (Bar.Close >= UpTrack) {
            msg = 'Buy Triggered at: ' + Bar.Close + ' Upper Track: ' + UpTrack;
            Log(msg);
