...
In addition, crossovers between price and Cloud Lines act as filters for trade signals. Only when both the Conversion/Price and price cross the cloud together will a valid signal be generated.
...
The biggest risk is that the Ichimoku system is sensitive to input parameters. Inappropriate settings may produce bad signals frequently. Also, the cloud tends to flatten during range-bound periods, causing uncertain signals. Frequent order openings and stops may incur large commission fees. Additionally, intermediate-term trades come with larger loss risks per trade, requiring strict risk control.
...
The Ichimoku Cloud strategy identifies intermediate-term trends using crossovers of the Conversion/Price Lines against the Cloud. Compared to single indicators, it incorporates multiple timeframes for reliable trend change detection. The inherent noise filtering also avoids whipsaws. With proper parameter tuning and risk management, this strategy can generate stable excess returns over the long run. It suits experienced trend traders with intermediate-term holding periods.
...
laggingSpan1 = donchian(laggingSpan2Periods - 26)

plot(conversionLine, title="Conversion Line", color=color.blue)
plot(baseLine, title="Base Line", color=color.orange)
plot(laggingSpan1, title="Lagging Span 1", color=color.red)
