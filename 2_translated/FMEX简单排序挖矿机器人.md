...
For detailed reference: https://www.fmz.com/digest-topic/5843
...
// Official mining coefficient, can be adjusted according to needs, such as setting it higher the further away from the order book to reduce transaction risk.
...
function calcDepth(){ 
    ...
    for(var i=0;i<15;i++){
        ...
        while(ask_price <= depth.Asks[i].Price){ // Considering unoccupied depth positions
            var my_ask_amount = _.findWhere(ordersInfo.sell, {price:ask_price}) ? _.findWhere(ordersInfo.sell, {price:ask_price}).amount : 0 // Exclude own orders' interference
