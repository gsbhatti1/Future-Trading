...
The Renko-Yin-Yang Quant Trading Strategy is a short-term trading strategy based on intraday price-volume relationship. This strategy utilizes the Yin Yang direction information within a day and combines volume confirmation signals to implement low-risk short-term operations.

## Strategy Logic

The strategy calculates the open, close, high, and low prices of each trading day, and generates Renko bricks together with the ATR indicator. Trading signals are generated when Yin Yang bricks reverse.
...
To filter false breakouts, the strategy also counts the number of periods for the last yin and yang lines. If the yin line has more periods, the signal is considered more reliable. Additionally, stop loss and take profit logic are set after buying and selling.

## Advantages

1. Renko bricks filter market noise, making trading signals clearer.
2. Combining price-volume relationships avoids the risk of false breakouts.
...
3. Too tight stop loss points may result in minor losses from retracements.

## Optimization

1. Consider combining other technical indicators to filter signals.
2. Consider adding trailing stop loss features.
3. Optimize parameters for different assets.
4. Consider combining different timeframes for multi-timeframe trading.

## Conclusion

In conclusion, this is a very practical short-term trading strategy. It uses price-volume relationships to efficiently filter and capture key turning points. Proper parameter tuning, risk management, and stop loss strategies can greatly improve its stability and profitability. With continuous optimization and testing, this strategy can become an essential tool for intraday traders.
...
// © dman103
strategy(title="Renko-Yin-Yang Quant Trading Strategy V2", shorttitle="Renko-Yin-Yang QS V2", overlay=true, precision=3, commission_value=0.025, default_qty_type=strategy.cash, default_qty_value=10000, initial_capital=10000)
