...
MACD with RR Ratio Intraday Convergence Strategy - MACD-Convergence-Strategy-with-RR-Daily-Limits-and-Tighter-Stop-Loss
...
// Determine long and short signals based on the value of MACD line
longCondition = (macdLine > longThreshold) and (macdLine > signalLine)
shortCondition = (macdLine < shortThreshold) and (macdLine < signalLine)

// Set take-profit and stop-loss levels for positions
takeProfitLong = strategy.position_avg_price + 600 * pointSize
stopLossLong = strategy.position_avg_price - 100 * pointSize

takeProfitShort = strategy.position_avg_price - 600 * pointSize
stopLossShort = strategy.position_avg_price + 100 * pointSize

// Define trailing stop-loss logic
trailStopLossLong = strategy.position_avg_price + (close - strategy.position_avg_price - 300) * pointSize
trailStopLossShort = strategy.position_avg_price - (strategy.position_avg_price - close - 300) * pointSize

// Set daily maximum loss and profit limits
dailyLimitLoss = 600 * pointSize
dailyLimitProfit = 1800 * pointSize

// Place orders based on signals
if (longCondition)
    strategy.entry("Long", strategy.long, when=true)
    strategy.exit("Exit Long", from_entry="Long", limit=takeProfitLong, stop=stopLossLong)

if (shortCondition)
    strategy.entry("Short", strategy.short, when=true)
    strategy.exit("Exit Short", from_entry="Short", limit=takeProfitShort, stop=stopLossShort)

// Handle trailing stops
strategy.exit("Trailing Stop Long", from_entry="Long", limit=trailStopLossLong)
strategy.exit("Trailing Stop Short", from_entry="Short", limit=trailStopLossShort)

// Close positions if daily limits are reached
if (strategy.closedtrades.profit > dailyLimitProfit or strategy.closedtrades.loss > dailyLimitLoss)
    for i = 1 to strategy.opentrades.count
        if (i == 1) // Assuming only one open trade for simplicity
