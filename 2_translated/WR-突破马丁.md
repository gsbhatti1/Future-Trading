...
|marginLevel|20|Magnification of leverage|
|baseAmount|10|Initial opening amount U|
...
def moveStocks(_moveStocks):
    global todayProfit
    if exchanges[0].GetName().find('Binance') >= 0: #Futures_Binance  Only futures with coin denomination and shared margin 2-U denomination to spot, 4-coin denomination to spot  returns tranId
        if exchanges[0].GetCurrency().find('USDT') >= 0:
            movecurrency = 'USDT'
            mtype = 2
        else: #coin denomination
            movecurrency = 'BTC'
            mtype = 4
        ...
        Log('Temporary unsupported exchange transfer')
...
def UpdateAccout():
    ...
    _G("allAccount",acc1 + acc2 + GetMargin()) #not calculated unrealized profit
    if acc2 < 1:
...
def initData():
    ...
    if exchanges[0].GetName().find('Binance') >= 0: #Futures_Binance
        ...
        Sleep(3000)
        Log("Exchange information retrieval completed")
    else:
        Log('Temporary unsupported exchange')
        return
