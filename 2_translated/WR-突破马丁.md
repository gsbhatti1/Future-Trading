...
|marginLevel|20|Magnification level|
|baseAmount|10|Initial opening U|
|WRperiod|14|WR period|
|zhiy|100|Take profit %|
|zhis|100|Stop loss %|
|autoMoveStocks|false|Automatically move stocks out when available assets exceed *U by 100U|
|maxBuc|10|Maximum doubling times|



|Button|Default|Description|
|----|----|----|
|Clear and Stop|__button__|Clear and stop|
...
def MyLog(str1, ktime, price=''):
    if (_G('str') != str1 or _G('ktime') != ktime) and not (_G('str2') == str1 and _G('ktime2') == _G('ktime')):
        _G('str2', _G('str'))
        _G('ktime2', _G('ktime'))
...
def moveStocks(_moveStocks):
    global todayProfit
    if exchanges[0].GetName().find('Binance') >= 0: #Futures_Binance only has delivery and shares a fund. Convert U-based to spot, or convert from spot to U-based.
        ...
        if moveid is not None:
            Log('Funds transfer successful', moveid)
            ...
            Log('Funds transfer failed')
    else:
        Log('Not supported for fund transfer on this exchange')
...
def coverAll(i):
    ...
    for j in range(len(position)):
        ...
        if position[j]["Type"] == 0:     # Holding long positions
            Deal(-1 , pamount, "closebuy", exchanges[i].GetCurrency() + 'manual market price close', i)
        elif position[j]["Type"] == 1:     # Holding short positions
            Deal(-1 , pamount, "closesell", exchanges[i].GetCurrency() + 'manual market price close', i)
...
def UpdateAccout():
    ...
    acc1 = accout.FrozenBalance   # frozen balance of other coins, is it shared?
    acc2 = accout.Balance
...
def Deal(price, num, btype, beizhu='', i=0):
    ...
    if btype == "closebuy" or btype == "sell":
        ...
        #Log('Short opening', price)
    else:
        exchanges[i].Buy(price, num, beizhu)
        #Log('Long opening', price)
...
def initData():
    ...
    if exchanges[0].GetName().find('Binance') >= 0: #Futures_Binance
        ...
        Sleep(3000)
        Log("End of obtaining exchange information")
    else:
        Log('Not supported for this exchange')
        return
    ...
    for i in range(len(exchanges)): # Initialize exchanges
