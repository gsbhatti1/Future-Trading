...
```pinescript
//@version=5
strategy("Key Reversal Backtest Strategy", overlay=false)

// Input Arguments
lookback = input(1, title="Lookback Bars for New Low")
takeProfitPips = input(20, title="Take Profit in Pips")
stopLossPips = input(10, title="Stop Loss in Pips")

// Function to identify key reversal day
isKeyReversalDay(upTrend, currentLow, previousClose) =>
    if (upTrend and currentLow < ta.lowest(lowest闭合符缺失，请稍后...)
```