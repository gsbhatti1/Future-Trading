...
long_condition = macd_line > 0 and macd_line[1] < 0 and abs(macd_line) > v_input_9
short_condition = macd_line < 0 and macd_line[1] > 0
...
// Trailing Stop Loss
long_entry = strategy.entry("Long", strategy.long, when = long_condition)
strategy.exit("Close Long", from_entry = "Long", stop = close - v_input_10 * close, limit = close + v_input_9 * close)

if (long_entry)
    strategy.close("Close Long", when=macd_line < 0 and signal_line > 0 and macd_line[1] < 0)
```

This Pine Script defines a MACD-based trading strategy with customizable parameters for the fast and slow moving averages, signal smoothing, and stop loss conditions. The script includes plotting of the MACD line, signal line, and color-coding to indicate the direction of the trend.