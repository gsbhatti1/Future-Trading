...
The fmz platform does not provide a take-profit and stop-loss function. When you need to set these functions, you have to call other functional interfaces of the exchange. The settings for take-profit and stop-loss vary between different exchanges, so the following packaging is made.
The stop-profit and stop-loss functions are particularly important. They avoid high handling fees without triggering by market price and also prevent liquidation in extreme circumstances.
These functions consider both isolated and full positions as well as U-standard currency standard.
...
# Perform take-profit and stop-loss on the contract cangType=0, default is isolated position =1 full position
