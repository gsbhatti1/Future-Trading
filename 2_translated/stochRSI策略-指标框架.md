...
Use Spot Cryptocurrency Trading Library
*/
...
var long = 1;
var free = 0;
var state = free; // Each open, close, reset 
...
var prefloatProfit = 0; // Previous floating profit, used for trailing stop
var openBalance = 0; // Opening balance
var isCover = false;
...
function openUpdate() { // Update after opening a position
    state = long;
    sellInfo = null;
}
function closeUpdate() { // Update after closing a position
    state = free;
    addLevel = 0;
    buyInfo = null;
...
function Calculate(nowAccount, nowDepth) { // Calculate and update profit/loss, floating profit, average price of holding
    ...
    NowPositionInfo.floatProfit = diff_balance + diff_stocks * nowDepth.Bids[0].Price; // Floating profit for this transaction
...
/*
LC := REF(CLOSE,1); // REF(C,1) is the closing price of the previous period
...
function FstochRSI(records, n, m, p1, p2) {
    ...
    //var LC = records[len - 2]; // Closing price of the previous period
    ...
    for (var a = 0; a < rsi.length; a++) { // Modify, do not use LLV
        for (var aa = 0; aa <= a; aa++) {
            rsi_a.push(rsi[aa]);
        }
        arr1.push(rsi[a] - TA.Lowest(rsi_a, m));
    }
    for (var b = 0; b < rsi.length; b++) { // Modify, do not use HHV
    ...
    */
    ...
    var c = 0;
    var diff = 0;
    if (arr3.length !== arr4.length) { // Actual test shows unequal lengths
...
It seems the code snippet was cut off at the end. Please provide the full or remaining part of the code so it can be completed accurately. If there is any specific section that needs further translation, let me know!