...
//Get the upper track array of the last K line
var midPrice = midLine[midLine.length -2];
//Get the middle track number of the last K line
var downPrice = downLine[ downLine.length -2];
//Get the lower track array of the previous K line
...
if (mp == 0 && close < midPrice) return -1; //If there is no position and the closing price is less than the lower track.Open short
});
}
```
