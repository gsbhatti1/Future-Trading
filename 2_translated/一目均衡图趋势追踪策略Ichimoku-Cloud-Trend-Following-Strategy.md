...
A buy signal is triggered when the conversion line crosses above the base line. A sell signal is triggered when it crosses below the base line. If the lagging span is above the price and the cloud color is green, a bull trend is indicated. 

It judges trend direction based on the relationship between the conversion and base lines. For example, when the conversion line breaks out above the base line, it signals a bullish trend. If the lagging span is also above the price, a long entry is triggered.

Set stop loss or take profit based on Leading Span A or Base Line. If the Base Line is chosen for stop loss, close position when price breaks below the Base Line.
...
Ichimoku Cloud combines indicators to judge market trend. It considers both short-term momentum and medium/long term trends. Conversion and Base Lines determine trading signals. The Base Line sets stop loss to lock in profits and control risks. This strategy suits medium to long term trend following.
