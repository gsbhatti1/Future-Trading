...
Huobi had an event for a period where trading in the HUSD market was fee-free, so I created a script to take advantage of this. The strategy is based on the principle that prices will always **revert to 1** and perform stable arbitrage.
...
|priceCeil|false|(?price interval) resistance level|
|priceFloor|false|support level|
|maxATR|0.0001|max average true range|
