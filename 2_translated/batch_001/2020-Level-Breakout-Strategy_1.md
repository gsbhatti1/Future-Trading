...
```
//@version=5
strategy("20-Level Breakout Strategy", overlay=true)
length = 20
ma = ta.sma(close, length)
percent_threshold = input.float(0.2, title="Threshold (%)")
macd_line = ta.macd(close)[1]

long_condition = close > ma * (1 + percent_threshold / 100) and macd_line >= 0
short_condition = close < ma * (1 - percent_threshold / 100) and macd_line <= 0
...
stop_loss_long = ma * (1 - 0.5 / 100)
take_profit_long = ma * (1 + 1 / 100)

stop_loss_short = ma * (1 + 0.5 / 100)
take_profit_short = ma * (1 - 1 / 100)
