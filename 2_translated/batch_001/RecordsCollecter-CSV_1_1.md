...
class Provider(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        try:
            ...
            # Note: period is the underlying K-line cycle.
            ...
            if isOnlySupportCSV:
                ...
                with open(filePathForCSV, "r") as f:
                    ...
                    if len(header) != len(data["schema"]):
                        Log("The CSV file format is incorrect, different number of columns. Please check!", "#FF0000")
                        return 
                    for ele in header:
            ...
            Log("Query conditions: ", dbQuery, "Number of queries:", exRecords.find(dbQuery).count(), "Total number of records in the database:", exRecords.find().count())
...
def main():
    ...
    if (isOnlySupportCSV):
        try:
            ...
            Log("Starting the custom data source service thread, data provided by CSV file.", "#FF0000")
        except BaseException as e:
    ...
    Log("Collecting K-line data from", exName, "exchange, ", "K-line cycle:", period, "seconds")
    
    # Connect to the database service, server address: mongodb://127.0.0.1:27017 (specific settings depend on the MongoDB installation on the server)
    Log("Connecting to the MongoDB service hosted by the provider, mongodb://localhost:27017")
    myDBClient = pymongo.MongoClient("mongodb://localhost:27017")   
    # Create a database
    ex_DB = myDBClient[exName]
