...
You can see the performance of my trades in the charts. For all my charts, visit: https://www.fmz.com/user/6261c777972a854f5c0460520f9206bd

You can also check my details and earnings rates on TraderWagon:
https://www.traderwagon.com/en/portfolio/3886?ref=zoh4wq9

By becoming a member of the trade platform called TraderWagon, which was established in partnership with Binance, you can copy my positions with one click at your own volume.
...
I make futures transactions on 4 or 5 coins that I have determined by examining detailed historical correlations. I also make changes in these coins when necessary.
...
|amountPrecision|2|Quantity precision|
...
function main() {
    ...
    // Switch to OKEX V5 simulation if needed
    ...
    while (1) {
        ...
        if (pos.length > 1) {
            Log(pos)
            throw "Multiple positions held at the same time"
        }
        // Based on position state
        if (pos.length == 0) {
            // No position, calculate and log profit
            if (!IsVirtual()) {
                var currTotalEq = getTotalEquity()
                if (currTotalEq) {
        ...
        } else if (pos[0].Type == PD_LONG) {   // Long position
```

Note that the function `LogVacuum()` and the variable `PD_LONG` are placeholders or specific to your trading environment. Ensure they match your actual implementation.