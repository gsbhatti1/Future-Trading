...
if inTimeRange and not hasTradedToday
    if high - low > candleSizeThreshold  // Long signal
        strategy.entry("Long", strategy.long, when=candleSize >= candleSizeThreshold)

    if low - high > candleSizeThreshold  // Short signal
        strategy.entry("Short", strategy.short, when=candleSize <= -candleSizeThreshold)

    hasTradedToday := true

// Exit condition based on take profit and stop loss
if isLongTrade
    strategy.exit("Take Profit Long", "Long", profit=takeProfit)
    strategy.exit("Stop Loss Long", "Long", loss=stopLoss)

if isShortTrade
    strategy.exit("Take Profit Short", "Short", profit=takeProfit)
    strategy.exit("Stop Loss Short", "Short", loss=stopLoss)
```

This completes the translation of your trading strategy document, keeping all code blocks and formatting as you requested.