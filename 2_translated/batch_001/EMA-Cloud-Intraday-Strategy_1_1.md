...
This strategy uses the 9 and 20 period exponential moving averages to form a colored cloud between them, similar to what is observed in the Ichimoku Cloud. All trades are closed at the end of the trading day. Entries occur when the price closes above a Green cloud (where 9 EMA is above 20 EMA) or below a Red cloud (where 9 EMA is below 20 EMA). Exits happen when the price closes against the 9 EMA or at the close of the trading day. Running the strategy tester across various intraday timeframes will reveal which timeframe works best for a specific symbol. For instance, I've found that this strategy yields optimal results for SPY on the 30-minute chart.
