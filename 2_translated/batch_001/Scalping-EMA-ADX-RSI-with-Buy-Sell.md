...
// Define the input and value where it is considered that there is a trend ready condition
trendReadyThreshold = input.int(title="Trend Ready Limit", defval=50, group='ADX Settings')
isTrendReady = sig >= trendReadyThreshold

// Buy/Sell Entry Rules
longCondition = ta.crossover(close, emaValue) and isRsiOS and isTrendReady
shortCondition = ta.crossunder(close, emaValue) and isRsiOB and isTrendReady

// Plot buy/sell signals on the chart
plotshape(series=longCondition, title="Buy Signal", location=location.belowbar, color=color.green, style=shape.labelup, text="BUY")
plotshape(series=shortCondition, title="Sell Signal", location=location.abovebar, color=color.red, style=shape.labeldown, text="SELL")

// Optional alerts for Buy, Sell or both
alertcondition(longCondition, title="Buy Alert", message="Buy Signal Generated")
alertcondition(shortCondition, title="Sell Alert", message="Sell Signal Generated")
```

This updated Pine Script includes the buy/sell entry rules based on the provided strategy description and completes the script.