...
3. Additionally, when the buy signal triggers, the stock price must be within 10% range of the two-year high, not too far away. It also needs to be below the 13-day line and above the 21-day line to ensure proper timing.
...
// Plot white up arrows whenever it's a new high
plotshape(newHigh, style=shape.triangleup, location=location.abovebar, color=color.white, size=size.tiny)
```