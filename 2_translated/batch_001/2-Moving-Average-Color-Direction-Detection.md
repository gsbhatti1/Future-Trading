...
A filling option has been added between the moving averages (it is deactivated by default, activate in configuration if desired). When both moving averages are green, the fill will be green; when both are red, the fill will be red. This confirms a healthy trend when using a fast and slow average. When the fast average changes direction, it will be filled with purple color, which may indicate: a trend correction is occurring, potential price pullback, possible range formation, or potential trend reversal. When the color turns purple, traders should analyze the overall context to determine what might be happening.

The following alerts have been created:
