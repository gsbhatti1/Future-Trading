...
timestart = timestamp(year(startY), month(startM), dayofmonth(startD), 0, 0)

// Trading window logic
if time < timestart
    strategy.close("Close all")

// ******************** Indicator Inputs **********************************
maType_oscillator = input.string(title="Oscillator MA Type", defval="EMA", options=["EMA", "SMA"], group="Indicators")
maType_signal = input.string(title="Signal Line MA Type", defval="EMA", options=["EMA", "SMA"], group="Indicators")

// ******************** Strategy Conditions ******************************
longCondition = crossover(price, ema(close, v_input_6)) and macd.macd_above(macd.signal)
shortCondition = crossunder(price, ema(close, v_input_6)) and macd.macd_below(macd.signal)

willyCondition = ta.crossover(willr(v_input_int_8), wma(willr(v_input_int_8), v_input_13))

// ******************** Entry/Exit Rules *********************************
if longCondition or shortCondition or willyCondition
    if not strategy.opentrades.exists("Long")
        strategy.entry("Long", strategy.long)

    if not strategy.opentrades.exists("Short")
        strategy.entry("Short", strategy.short)

// ******************** Stop Loss and Take Profit ************************
atrPeriod = v_input_int_5
atrValue = ta.atr(atrPeriod)
stopLossLevel = atrValue * v_input_float_3

if isLongPosition
    strategy.exit("Take Profit Long", from_entry="Long", limit=price + stopLossLevel)

if isShortPosition
    strategy.exit("Take Profit Short", from_entry="Short", limit=price - stopLossLevel)

// ******************** Plotting and Colors *******************************
plotshape(series=longCondition, title="Long Entry", location=location.belowbar, color=color.rgb(29, 98, 255), style=shape.labelup, text="BUY")
plotshape(series=shortCondition, title="Short Entry", location=location.abovebar, color=color.rgb(255, 109, 0), style=shape.labeldown, text="SELL")

bgcolor(v_input_7, transp=true)
line.new(x1=timestamp(year(startY), month(startM), dayofmonth(startD), 0, 0), y1=v_input_4, x2=timestamp(finishY, finishM, finishD, 0, 0), y2=v_input_4, color=color.rgb(56, 223, 187), width=2)
```

Note: The original Pine Script included incomplete logic and missing variable definitions. This translated script includes placeholders for the missing parts to maintain consistency with the original structure.