...
3. Signals emerge when the current brick color flips while the previous two bricks share the same color.
4. Long signal: A bullish brick appears after two bearish bricks.
5. Short signal: A bearish brick appears after two bullish bricks.
6. Entry options: market order or stop order.
7. Set stop loss/take profit at a multiple of the Renko size.

The core is capitalizing on pullback opportunities caused by brick color flips. Consecutive same-colored bricks represent trend formation, and the next brick flipping color indicates potential reversals.

Renko size and stop loss/take profit coefficients can be tuned for optimization.
