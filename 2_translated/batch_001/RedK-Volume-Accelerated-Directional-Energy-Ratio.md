...
The Volume-Accelerated Directional Energy Ratio (VADER) utilizes price movements and the associated volume to estimate the positive (buying) and negative (selling) "energy" behind the scenes, enabling traders to "read the market action" in more detail and adjust their trading decisions accordingly.
...
Large price moves accompanied by large volume indicate serious buyers (if the move is up) or sellers (when the move is down), who are in control of the market.
On the other hand, when price moves are small but with large volume, it suggests a fight or balance of energy between buying and selling forces.
Also, when large price moves are accompanied by relatively low volume, there's a lack of "energy" from both buyers and sellers—such moves are usually short-lived.

The analogy with VADER is that we look at the price change (difference in closing prices over two bars) as the displacement (or action result), and the associated volume as the "effort" behind this action. Combining these two values, the displacement and effort, gives us a representation or proxy of the underlying energy (in a specific direction). When both values are high, then the resulting energy is high; if one of these values is low, the resulting energy is low.
...
Note that we're approaching this analogy from a trading perspective and not a physics perspective. We can be forgiven for differences in the energy calculation between physics and trading.
