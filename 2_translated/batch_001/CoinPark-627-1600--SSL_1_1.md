...
CoinPark Exchange General Protocol - 627-1600 - Update - SSL Verification Closed
...
## CoinPark General Protocol Python 2 Version
Purpose: To enable BotVs to support the coinpark.cc exchange.
Code Public Address: https://www.fmz.com/strategy/101399
Running Address:

    http://127.0.0.1:6667
### 1. Run Plugin on the Host Server

On the server where the host is saved, run in the background:

    nohup python coinpark.py &
You can also specify a port when running:

    python coinpark.py 8866
It can also be run as an ordinary robot on the simulation disk without charging any fees.

### 2. Configure Exchange on FMZ Website

Exchange configuration is as follows:
![iamge](https://dn-filebox.qbox.me/c1456599386e597f2966f67b49c7102bab857fef.png)
Since there are no assets in the account, detailed testing was not performed. Bug reports are welcome.
Updated on June 26, 2018, 3:57 PM, fixed a bug.
...
# -*- coding: utf-8 -*-
'''
CoinPark General Protocol, using Python 2.7
Running Address: http://127.0.0.1:6667, port can be specified
Since there are no assets in the account, detailed testing was not performed. Bug reports are welcome.
QQ:1051804485
Feedback Address: https://www.botvs.com/bbs-topic/1963
Updated on June 26, 2018, 3:57 PM, fixed a bug
The general protocol can be run as an ordinary robot on the BotVs simulation disk without charging any fees
To use IO functions, you need to override the rpc method in exchange. An example in JavaScript is as follows:
