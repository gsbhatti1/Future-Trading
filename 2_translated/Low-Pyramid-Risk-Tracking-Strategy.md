```markdown
---
Name: Low-Pyramid-Risk-Tracking-Strategy
...
### Low Point Identification Module
This module uses the difference between RSI and its EMA to identify potential price lows. To improve accuracy, moving average indicator and multi-timeframe stochastic indicators are introduced for signal filtering. Only when the price is below the moving average and the stochastic K line is below 30 will the validity of the low point signal be confirmed.

### Pyramid Tracking Module
This is the core of this strategy. Once the low point signal is confirmed, the strategy will open the first position at 0.1% below that low point. Afterwards, as long as the price continues to fall and is below a certain percentage of the average entry price, more long orders will be added. The size of new orders will increase in sequence, for example, the third order is three times the size of the first order. This pyramid tracking approach helps in averaging risks. The strategy allows up to 12 tracking orders.

### Risk Control Module
This module includes three aspects: 
- Overall stop loss based on highest price in recent periods. All orders will follow this stop loss.
- Independent take profit setting for each order, which allows closing the order based on a certain percentage of the entry price.
- Overall stop loss based on equity percentage, which is the strongest risk control method.
...
This strategy effectively reduces risks of individual orders through pyramid tracking approach, and overall stop loss, take profit, trailing stop functions also play very good role in risk control. But there is still room for improving low point identification and other aspects. If more advanced techniques can be introduced, dynamic adjustment of parameters can be added, combined with parameter optimization, the strategy's risk-reward ratio will significantly improve.
```