...
The moving base-level helps to reduce the number of false signals. In an uptrend, the base-level is below zero; in a downtrend, it is above zero. This allows us to take into account the trend stability effect. In this case, to form a reversal signal, the oscillator must cross a lower value in an uptrend and a higher value in a downtrend.
