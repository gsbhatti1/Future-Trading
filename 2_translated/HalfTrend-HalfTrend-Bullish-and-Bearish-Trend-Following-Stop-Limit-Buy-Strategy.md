...
#### Strategy Principle
1. Calculate the HalfTrend indicator value, which requires setting the lookback period `length` and amplitude parameter `amplitude`.
...
#### Strategy Risks
1. Trend reversal determination relies on the amplitude parameter, and inappropriate parameter values may lead to premature or delayed order placement.
...
//@version=5
strategy("HalfTrend Multi-Trend Stop-Limit Buy", overlay=true)
...
if (buySignal)
    ...
    strategy.entry("HalfTrend Buy", strategy.limit, stop=stopPrice)
```
