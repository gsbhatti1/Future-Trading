...
SuperHunter - Intelligent Learning - Multi-Account Combined Display Edition
...
function printPositionInfo(exchangeInnerArray, totalProfitUSDT, totalProfitRate) {
    ...
    table.rows.push([{
        body: 'This strategy is USDT-centric, a mean reversion arbitrage strategy on Binance futures contracts, with low-risk auxiliary grids running in parallel (BitMEX supports BTC-centric)',
        colspan: 5
    }]);
    table.rows.push([{
        body: 'The main arbitrage currencies are BTC/USDT and ETH/USDT, and the grid covers all trading pairs of Binance perpetual futures',
        colspan: 5
    }]);
