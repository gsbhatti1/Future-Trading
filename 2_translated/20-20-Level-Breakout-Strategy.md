...
// Check if the closing price breaks through the base line and meets MACD condition
breakoutConditionLong = close > baseLine * 1.002 and ta.crossover(macd, macdsignal)
breakoutConditionShort = close < baseLine * 0.998 and ta.crossunder(macd, macdsignal)

// Set stop loss and take profit levels for long positions
longStopLoss = baseLine * 0.995
longTakeProfit = baseLine * 1.01

// Set stop loss and take profit levels for short positions
shortStopLoss = baseLine * 1.005
shortTakeProfit = baseLine * 0.99

// Enter long position when breakout condition is met and MACD condition is satisfied
if (breakoutConditionLong)
    strategy.entry("Long", strategy.long, when=bar_index == 1)

// Enter short position when breakout condition is met and MACD condition is satisfied
if (breakoutConditionShort)  
    strategy.entry("Short", strategy.short, when=bar_index == 1)
